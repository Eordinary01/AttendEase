...
# Heading size mapping from reference docs
...
def add_heading(doc, text, level, ref_h_sizes):
    if level == 1:
        size_pt = ref_h_sizes[1]
        bold = True
    elif level == 2:
        size_pt = ref_h_sizes[2]
        bold = True
    elif level == 3:
        size_pt = ref_h_sizes[3]
        bold = True
    else:
        size_pt = 12
        bold = False
...
# Add TOC entries with tab stops for page numbers
toc_entries = [
    ("Introduction", "1"),
