...
def add_heading(doc, text, level, ref_doc):
    if level == 1:
        size_pt = Pt(ref_doc.styles['Heading 1'].font.size).pt if ref_doc.styles['Heading 1'].font.size else 18
        bold = ref_doc.styles['Heading 1'].font.bold
    elif level == 2:
        size_pt = Pt(ref_doc.styles['Heading 2'].font.size).pt if ref_doc.styles['Heading 2'].font.size else 16
        bold = ref_doc.styles['Heading 2'].font.bold
    elif level == 3:
        size_pt = Pt(ref_doc.styles['Heading 3'].font.size).pt if ref_doc.styles['Heading 3'].font.size else 14
        bold = ref_doc.styles['Heading 3'].font.bold
    else:
        size_pt = 12
        bold = False
...
# Add new section for rest of report
...
while i < len(lines):
    ...
    if stripped.startswith("# "):
        flush_table(rpt_doc)
        add_heading(rpt_doc, stripped.lstrip("# ").strip(), 1, ref_rpt_doc)
    elif stripped.startswith("## "):
        flush_table(rpt_doc)
        add_heading(rpt_doc, stripped.lstrip("# ").strip(), 2, ref_rpt_doc)
    elif stripped.startswith("### "):
        flush_table(rpt_doc)
        add_heading(rpt_doc, stripped.lstrip("# ").strip(), 3, ref_rpt_doc)
    elif stripped.startswith("#### "):
        flush_table(rpt_doc)
        add_heading(rpt_doc, stripped.lstrip("# ").strip(), 4, ref_rpt_doc)
